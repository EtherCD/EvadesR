...
export default class Entity {
	...
	draw(ctx: CanvasRenderingContext2D, delta: number) {
		...
		ctx.fillStyle = GlobalAssets.entities[this.type].color;
	...
	drawaura(ctx: CanvasRenderingContext2D) {
		...
		if (this.aura > 0) ctx.arc(Camera.w / 2 + (this.x - Camera.x) * Camera.s, Camera.h / 2 + (this.y - Camera.y) * Camera.s, this.aura * Camera.s, 0, Math.PI * 2);
		ctx.fill();
		ctx.globalAlpha = 1;
		ctx.closePath();
	}
