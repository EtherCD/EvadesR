			...
			{worldList.map((world, i) => (
				<div key={i}>
					<div className={styles.leaderboardTitleBreak}>
						<span style="colorize(c.color)">{world.name}</span>
					</div>
					{world.players.map((player, i) => (
						<div className={styles.leaderboardLine} key={i}>
							{player.self ? (
								<b style={{ color: player.died ? "red" : world.color }} v-if="p.self">
									{player.name}[{player.area}]{player.died && <label>• {player.timer}</label>}
								</b>
							) : (
