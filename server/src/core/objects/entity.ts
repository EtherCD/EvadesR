...
export abstract class Entity {
	...
	constructor(data: EntityProps) {
		...
		this.area = data.area;
		this.pos = [random(this.radius, this.area.w - this.radius), random(this.radius, this.area.h - this.radius)];
		this.angle = Math.random();
		this.vel = [Math.cos(this.angle * Math.PI * 2) * data.speed, Math.sin(this.angle * Math.PI * 2) * data.speed];
	...
	interact(player: Player) {
		if (!player.immortal && !player.downed) {
			if (distance(player.pos[0], player.pos[1], this.pos[0], this.pos[1]) <= this.radius + player.radius && !this.harmless) {
				player.knock();
			}
			if (this.aura != undefined && this.aura > 0) {
				if (distance(player.pos[0], player.pos[1], this.pos[0], this.pos[1]) <= this.aura + player.radius) {
